...
export default function AutoFillPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('discom', images.discom);
    formData.append('netMeter', images.netMeter);
    formData.append('location', images.location);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-yellow-100 py-10">
      <div className="w-full max-w-xl p-8 rounded-3xl shadow-2xl bg-white/90 border border-yellow-200 backdrop-blur-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block font-medium mb-1">1. National Portal Screenshot</label>
            <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'discom')} />
          </div>
          <div>
            <label className="block font-medium mb-1">2. NET METERING SCREENSHOT FROM TGSPDCL</label>
            <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'netMeter')} />
          </div>
          <div>
            <label className="block font-medium mb-1">3. Location Screenshot</label>
            <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'location')} />
          </div>
          <div>
            <label className="block font-medium mb-1">4. Panel Label Photo</label>
            <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'pvModule')} />
          </div>
          <div>
            <label className="block font-medium mb-1">5. Inverter Screenshot</label>
            <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'inverter')} />
          </div>
